...
Die App generiert keinen Python-Code aus Admin-Eingaben. Admins bearbeiten Dienste, Systeme, Schritte, Lösungen, Regeln und Entscheidungsnetze über die Oberfläche. Gespeichert wird alles als JSON im Ordner `Rule Engine/`. Die Python-Rule-Engine interpretiert diese Technische JSON-Dateien zur Laufzeit.
...
- **Wissensbausteine** als zentraler Katalog mit technischer Condition-ID, Anzeigename, Wissensbereich, Kategorie, erlaubten Werten und Regelverwendung.
...
## Update: Testfall-Fixes nach Excel-Bewertungen

Die Nutzer- und Dialoglogik wurde anhand der dokumentierten Testfälle nachgeschärft. Wichtige Verbesserungen:

- Datenschutzfreundliche Antworten: Der Assistent fordert keine konkreten Benutzernamen, Kürzel, Passwörter oder API-Keys im Chat an.
- Konsistenter Du-Stil und weniger technische Sprache in Nutzerantworten.
- Interne Begriffe wie Rule Engine, Fakten oder Gruppen-Inferenztest werden in normalen Nutzerantworten ausgeblendet.
- Robustere Erkennung von eduroam-Sichtbarkeit, eduroam verbunden ohne Internet, Profil gelöscht und Zertifikatswarnungen.
- MFA-Fälle wurden erweitert: Handy/2FAS-Einrichtung, abgelaufener oder ungültiger 30-Sekunden-Code, verlorenes Handy/kein Zugriff auf 2FA und QR-Code-Probleme.
- VPN-Fälle wurden erweitert: MFA-Code beim VPN-Login, Benutzername-/Passwortformat, Client startet nicht, Installation schlägt fehl, Zertifikatswarnung und VPN verbunden aber kein Internet.
- Mehrthemen-Anfragen werden erkannt und geklärt, statt automatisch ein falsches Thema zu erzwingen.
- Wirre oder Troll-Eingaben werden abgefangen und der Nutzer wird gebeten, eine echte IT-Frage zu stellen.
- ILIAS/Etherpad-Lite wird als eigenes Thema erkannt und sinnvoll eingeordnet.


## Update: Adminsicht und Regel-Excel v2

Die Adminsicht wurde an das verbesserte Admin-Konzept und die Datei `Regeln_vollständig_neu.xlsx` angepasst. Die fachlichen Pflegebereiche heißen nun `Wissensbereiche`, `Wissensbausteine`, `Funktionen & Antworten`, `Regelverwaltung`, `Abläufe & Netz` und `Wissensmodell`.

Die Excel-Inhalte wurden zusätzlich als technische JSON-Referenzen unter `Rule Engine/technical/` abgelegt und in die Kataloge für Wissensbausteine, Funktionen, Abläufe und Regeln übernommen. JSON bleibt als technische Vorschau erhalten, ist aber nicht mehr der primäre Pflegeeinstieg.
