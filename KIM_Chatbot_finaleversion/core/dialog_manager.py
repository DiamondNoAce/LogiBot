...
def _support_fallback_text(facts: dict[str, Any] | None = None) -> str:
    """Einheitlicher Fallback-Text mit optionaler Support-Zusammenfassung.

    Der Nutzer erhält keine beliebige Anleitung, sondern eine kurze Klärung und
    bei Bedarf direkt eine brauchbare Zusammenfassung für den Support.
    """
    facts = facts or {}
    service = str(facts.get("service") or facts.get("topic") or "").strip()
    system = _wizard_system_from_facts(facts)
    problem = str(
        facts.get("problem_type")
        or facts.get("eduroam_problem_type")
        or facts.get("vpn_problem_type")
        or facts.get("mfa_problem_type")
        or facts.get("intent")
        or ""
    ).strip()

    summary_lines: list[str] = []
    if service and service not in {"unknown", "none", "mixed"}:
        summary_lines.append(f"- erkannter Dienst: {_service_display_name(service)}")
    if system != "unknown":
        summary_lines.append(f"- erkanntes Betriebssystem/Gerät: {system}")
    if problem and problem not in {"unknown", "none"}:
        summary_lines.append(f"- erkannter Problemkontext: {problem}")
    if facts.get("password_recently_changed") is True:
        summary_lines.append("- Hinweis: Passwort wurde kürzlich geändert")

    base = (
        "Ich kann dafür keine ausreichend sichere passende Anleitung aus der Wissensbasis auswählen. "
        "Bitte ergänze Dienst, Gerät/Betriebssystem und was genau passiert. "
        "Wenn du nicht weiterkommst oder der Fall nicht in der Wissensbasis enthalten ist, wende dich bitte an den KIM-IT-Service-Desk."
    )
    if summary_lines:
        return base + "\n\nFür den Support kannst du diese Informationen mitgeben:\n" + "\n".join(summary_lines)
    return base


def _setup_package_for_service_system(service_key: str, system_key: str) -> str | None:
    """Liefert das passende Schrittpaket für eine explizite Neuinstallation/Einrichtung.

    Dieser sichere Router verhindert, dass nach einer Nutzerbitte wie
    „hilf mir beim erneuten Installieren“ ein beliebiges System geraten wird.
    Erst wenn das Betriebssystem bekannt ist, wird das passende Paket ausgewählt.
    """
    system_key = _wizard_system_from_facts({"os": system_key})
    package_map = {
        "eduroam": {
            "windows": "steps.eduroam.windows_cat",
            "macos": "steps.eduroam.macos_cat_profile",
            "linux": "steps.eduroam.linux_cat_script",
            "android": "steps.eduroam.android_geteduroam",
            "chromeos": "steps.eduroam.chromeos_onc",
            "ios": "steps.eduroam.ios_cat_profile",
            "ipados": "steps.eduroam.ios_cat_profile",
        },
        "vpn": {
            "windows": "steps.vpn.windows_install",
            "macos": "steps.vpn.macos_install",
            "linux": "steps.vpn.linux_install",
            "android": "steps.vpn.android_setup",
            "ios": "steps.vpn.ios_setup",
            "ipados": "steps.vpn.ios_setup",
        },
    }
    package_id = package_map.get(str(service_key or "").lower(), {}).get(system_key)
    if package_id and kb_json.get_step_package(package_id):
        return package_id
    return None


def _result_for_ask(fact: str, text: str, facts: dict[str, Any], *, rule_id: str = "dialog.safe_clarification") -> dict[str, Any]:
    action = {"type": "ask", "fact": fact, "text": text}
    return {
        "facts": dict(facts or {}),
        "matched_rules": [{
            "rule_id": rule_id,
            "module": "dialog_guard",
            "rule_group": "safe_clarification",
            "description": "Sichere Rückfrage statt unsicherer Anleitungsauswahl",
...
def _result_for_support_fallback(facts: dict[str, Any], *, rule_id: str = "dialog.support_fallback") -> dict[str, Any]:
    action = {"type": "answer", "text": _support_fallback_text(facts)}
    return {
        "facts": dict(facts or {}),
        "matched_rules": [{
            "rule_id": rule_id,
            "module": "dialog_guard",
            "rule_group": "support_fallback",
            "description": "Fallback auf Klärung und Supportverweis",
            "source": "dialog_manager",
            "actions": [action],
            "stop_after_match": True,
        }],
...
def _result_for_show_steps(package_id: str, facts: dict[str, Any], *, reason: str) -> dict[str, Any]:
    package = kb_json.get_step_package(package_id)
    action = {"type": "show_steps", "step_package_id": package_id, "step_package": package}
    return {
        "facts": dict(facts or {}),
        "matched_rules": [{
            "rule_id": "dialog.safe_setup_package",
            "module": "dialog_guard",
            "rule_group": "safe_setup_routing",
            "description": reason,
            "source": "dialog_manager",
            "actions": [action],
            "stop_after_match": True,
...
def _has_user_visible_action(result: dict[str, Any]) -> bool:
    return any(action.get("type") in {"ask", "answer", "recommend", "redirect_topic", "show_steps", "function"} for action in result.get("actions", []) or [])


def _action_types(result: dict[str, Any]) -> list[str]:
    return [str(action.get("type")) for action in result.get("actions", []) or [] if action.get("type")]


def _has_show_steps_action(result: dict[str, Any]) -> bool:
    return any(action.get("type") == "show_steps" for action in result.get("actions", []) or [])


def _service_is_known(facts: dict[str, Any]) -> bool:
    service = str(facts.get("service") or facts.get("topic") or "unknown").lower()
    return service not in {"", "unknown", "none", "mixed"}


def _intent_is_known(facts: dict[str, Any]) -> bool:
    intent = str(facts.get("intent") or "unknown").lower()
    return intent not in {"", "unknown", "none", "invalid_or_unclear"}


def _result_for_unknown_request_flow(facts: dict[str, Any], *, repeated: bool = False) -> dict[str, Any]:
    guarded = dict(facts or {})
    guarded["unknown_request_flow"] = True
    guarded["unknown_request_attempts"] = int(guarded.get("unknown_request_attempts") or 0) + 1
    if repeated:
        return _result_for_support_fallback(guarded, rule_id="dialog.unknown_request_support_fallback")
    action = {
        "type": "ask",
        "fact": "service",
        "text": (
            "Ich kann dein Anliegen noch keinem sicheren IT-Bereich zuordnen. "
            "Geht es um eduroam/WLAN, VPN, MFA/Authenticator, Benutzerkonto/Passwort oder etwas anderes? "
            "Beschreibe bitte kurz den Dienst, dein Gerät/Betriebssystem und was genau passiert."
        ),
    }
    return {
        "facts": guarded,
        "matched_rules": [{
            "rule_id": "dialog.unknown_request_flow",
            "module": "dialog_guard",
            "rule_group": "unknown_request_flow",
            "description": "Unklare Anfrage: erst fachliche Zuordnung abfragen, nicht raten",
            "source": "dialog_manager",
            "actions": [action],
            "stop_after_match": True,
        }],
...
def _result_for_mixed_topic_clarification(facts: dict[str, Any]) -> dict[str, Any]:
    guarded = dict(facts or {})
    mentioned = guarded.get("mentioned_services") or []
    service_labels = []
    for service_key in mentioned:
        try:
            service_labels.append(_service_display_name(str(service_key)))
        except Exception:
            service_labels.append(str(service_key))
    label_text = ", ".join(service_labels) if service_labels else "mehrere Dienste"
    action = {
        "type": "ask",
        "fact": "service",
        "text": (
            f"Ich habe {label_text} erkannt. Damit ich keine falsche Anleitung auswähle: "
            "Für welchen Dienst möchtest du zuerst Hilfe? Bitte antworte z. B. mit eduroam, VPN, MFA oder Benutzerkonto."
        ),
    }
    return {
        "facts": guarded,
        "matched_rules": [{
            "rule_id": "dialog.mixed_topic_clarification",
            "module": "dialog_guard",
            "rule_group": "safe_clarification",
            "description": "Mehrere mögliche Dienste erkannt; Dienst zuerst klären",
            "source": "dialog_manager",
            "actions": [action],
            "stop_after_match": True,
        }],
...
def _result_for_intent_clarification(facts: dict[str, Any]) -> dict[str, Any]:
    guarded = dict(facts or {})
    service_key = str(guarded.get("service") or guarded.get("topic") or "Dienst")
    service_label = _service_display_name(service_key) if service_key not in {"", "unknown"} else "dem Dienst"
    action = {
        "type": "ask",
        "fact": "intent",
        "text": (
            f"Ich habe {service_label} erkannt, aber noch nicht sicher, was du damit machen möchtest. "
            "Möchtest du etwas neu einrichten, ein bestehendes Problem beheben oder nur eine Information bekommen?"
        ),
    }
    return {
        "facts": guarded,
        "matched_rules": [{
            "rule_id": "dialog.intent_clarification",
            "module": "dialog_guard",
            "rule_group": "safe_clarification",
            "description": "Dienst erkannt, aber Absicht noch unklar",
            "source": "dialog_manager",
            "actions": [action],
            "stop_after_match": True,
        }],
...
def build_decision_diagnosis(
    ...
    *,
    ...
    """Erzeugt eine verständliche Admin-Diagnose zur Antwortauswahl.

    Die Diagnose ist absichtlich nicht Teil der normalen Nutzerantwort. Sie soll
    Admins zeigen, ob die Anfrage sicher genug erkannt wurde und warum eine
    Rückfrage, Anleitung oder ein Support-Fallback gewählt wurde.
    """
    service = str(facts.get("service") or facts.get("topic") or "unknown").lower()
    intent = str(facts.get("intent") or "unknown").lower()
    system_key = _wizard_system_from_facts(facts)
    actions = _action_types(result)
    matched_rules = result.get("matched_rules", []) or []
    evaluated_rules = result.get("evaluated_rules", []) or []
    reasons: list[str] = []
    missing: list[str] = []
    risks: list[str] = []
    score = 0.0

    if service not in {"", "unknown", "none", "mixed"}:
        score += 0.30
        reasons.append(f"Dienst erkannt: {_service_display_name(service)}")
    elif service == "mixed":
        risks.append("Mehrere Dienste erkannt; eindeutige Zuordnung fehlt.")
        missing.append("Dienst priorisieren")
    else:
        risks.append("Kein Dienst sicher erkannt.")
        missing.append("Dienst, z. B. eduroam, VPN, MFA oder Benutzerkonto")

    if intent not in {"", "unknown", "none", "invalid_or_unclear"}:
        score += 0.18
        reasons.append(f"Absicht erkannt: {intent}")
    else:
        missing.append("Absicht, z. B. Einrichten, Problem beheben oder Information")

    if system_key != "unknown":
        score += 0.14
        reasons.append(f"Betriebssystem/Gerät erkannt: {system_key}")
    elif service in {"eduroam", "vpn"} and (intent == "setup" or facts.get("awaiting_setup_os")):
        risks.append("Für die Einrichtung ist das Betriebssystem nötig; ohne OS wäre die Anleitung unsicher.")
        missing.append("Betriebssystem/Gerät")

    if matched_rules:
        score += 0.18
        reasons.append(f"{len(matched_rules)} Regel(n) gematcht.")
    else:
        risks.append("Keine Regel wurde gematcht.")

    if actions:
        score += 0.15
        reasons.append("Nächste Aktion vorhanden: " + ", ".join(actions))
    else:
        risks.append("Keine nutzerseitig sichtbare Aktion gefunden.")

    if _has_show_steps_action(result):
        # Schrittpakete sind nur sicher, wenn Dienst und ggf. OS bekannt sind.
        if service in {"eduroam", "vpn"} and system_key == "unknown":
            score -= 0.35
            risks.append("Schrittpaket würde ohne eindeutiges Betriebssystem starten.")
        else:
            score += 0.10
            reasons.append("Schrittpaket/Anleitung kann konkret gestartet werden.")

    if facts.get("invalid_request"):
        score = min(score, 0.10)
        risks.append("Eingabe wirkt wie Test/Nonsense oder ist fachlich nicht auswertbar.")

    if override_status:
        reasons.append(f"Sicherheitslogik: {override_status}")
    if fact_status:
        reasons.append(f"Faktenerkennung: {fact_status}")
    if reply_status:
        reasons.append(f"Antwortstatus: {reply_status}")

    score = max(0.0, min(1.0, score))
    first_rule = matched_rules[0] if matched_rules else {}
    if first_rule.get("rule_group") == "support_fallback" or "support_fallback" in str(matched_rules):
        score = min(score, 0.35)
        risks.append("Die Sicherheitslogik hat keine ausreichend sichere Fachanleitung zugelassen.")
        status = "kein Treffer"
        next_action = "Support-Fallback ausgeben"
    elif any(action == "ask" for action in actions):
        status = "teilweise erkannt" if score >= 0.35 else "zu unklar"
        next_action = "Rückfrage stellen"
    elif score >= 0.75:
        status = "sicher erkannt"
        next_action = "Passende Antwort/Anleitung ausgeben"
    elif score >= 0.40:
        status = "teilweise erkannt"
        next_action = "Nur sichere nächste Aktion ausführen"
    elif not actions or not matched_rules:
        status = "kein Treffer"
        next_action = "Support-Fallback ausgeben"
    else:
        status = "zu unklar"
        next_action = "Klärende Rückfrage stellen"

    return {
        "status": status,
        "score": round(score, 2),
        "next_action": next_action,
        "erkannter_dienst": service,
        "erkannte_absicht": intent,
        "erkanntes_betriebssystem": system_key,
        "actions": actions,
        ...
        "dialog_context": build_dialog_context(facts, result),
    }


def _diagnosis_line(diagnosis: dict[str, Any]) -> str:
    if not diagnosis:
        return ""
    return f"Sicherheit: {diagnosis.get('status')} ({diagnosis.get('score')}) · nächste Aktion: {diagnosis.get('next_action')}"


def _known_text(value: Any, default: str = "unbekannt") -> str:
    if value is None:
        return default
...
def build_dialog_context(
    ...
    *,
    ...
    """Verdichteter Gesprächskontext für laufende Dialoge.

    Der Kontext ist kein eigener Entscheidungsmechanismus, sondern macht sichtbar,
    welche Informationen aus früheren Nachrichten weiterhin gelten und welche
    Information gerade noch offen ist. Dadurch werden Folgeantworten wie
    „hilf mir beim Neuinstallieren“ im vorhandenen eduroam-Kontext gehalten.
    """
    facts = dict(facts or {})
    result = result or {}
    service = str(facts.get("service") or facts.get("topic") or "unknown").lower()
    ...
    problem = (
        ...
        or facts.get("vpn_problem_type")
        or facts.get("mfa_problem_type")
        or facts.get("problem_area")
        or "unknown"
    )

    hints: list[str] = []
    if facts.get("service_previously_used") is True or facts.get("service_previously_worked") is True:
        hints.append("Dienst wurde laut Nutzer vorher bereits verwendet.")
    if facts.get("password_recently_changed") is True:
        hints.append("Passwort wurde kürzlich geändert.")
    if facts.get("profile_recreation_required") is True or facts.get("awaiting_setup_os") is True:
        hints.append("Neueinrichtung/Profil-Neuerstellung ist im Gesprächskontext aktiv.")
    if facts.get("invalid_request") is True:
        hints.append("Eingabe wirkt fachlich nicht auswertbar.")
    guard_notes = facts.get("__llm_guard_notes") or []
    if guard_notes:
        hints.append(f"LLM-Guard hat {len(guard_notes)} Wert(e) geprüft/verworfen.")

    action_types = _action_types(result) if result else []
    next_action = "keine"
    if any(a == "ask" for a in action_types):
        next_action = "Rückfrage stellen"
    elif any(a == "show_steps" for a in action_types):
        next_action = "Schrittpaket/Anleitung starten"
    elif any(a in {"answer", "recommend", "redirect_topic", "function"} for a in action_types):
        next_action = "Regelbasierte Antwort ausgeben"

    return {
        "dienst": _service_display_name(service) if service not in {"", "unknown", "none", "mixed"} else "unbekannt",
        "dienst_key": service,
        "absicht": _known_text(intent),
        "betriebssystem": system_key if system_key != "unknown" else "unbekannt",
        "problemkontext": _known_text(problem),
        "bekannte_hinweise": hints,
        "offene_information": pending_fact or _pending_ask_fact(result) or None,
        "letzte_rueckfrage": pending_question or _pending_question_text(result) or None,
        "naechste_aktion": next_action,
        "support_zusammenfassung": _support_fallback_text(facts),
        "llm_guard_notes": guard_notes,
    }


def _attach_dialog_context(session: dict[str, Any], facts: dict[str, Any], result: dict[str, Any]) -> None:
    session["dialog_context"] = build_dialog_context(
        facts,
        result,
...
def build_graph_diagnosis(
    ...
    *,
    ...
    """Diagnose für den Entscheidungsnetz-Test."""
    service = str(facts.get("service") or facts.get("topic") or "unknown").lower()
    intent = str(facts.get("intent") or "unknown").lower()
    system_key = _wizard_system_from_facts(facts)
    graph_status = str(result.get("status") or "unknown")
    path = result.get("path", []) or []
    edges = result.get("evaluated_edges", []) or []
    reasons: list[str] = []
    risks: list[str] = []
    missing: list[str] = []
    score = 0.0
    ...
    if graph:
        reasons.append(f"Graph gewählt: {graph.get('name') or graph.get('id')}")
        score += 0.10
    if service not in {"", "unknown", "none", "mixed"}:
        reasons.append(f"Dienst erkannt: {_service_display_name(service)}")
        score += 0.20
    ...
    if graph_status == "question":
        score += 0.25
        reasons.append("Entscheidungsnetz hat gezielt eine Rückfrage erzeugt.")
        if result.get("fact") or result.get("expected_fact"):
            missing.append(str(result.get("fact") or result.get("expected_fact")))
        next_action = "Rückfrage aus dem Entscheidungsnetz stellen"
        status = "teilweise erkannt" if score >= 0.35 else "zu unklar"
    elif graph_status == "terminal":
        score += 0.35
        terminal = result.get("terminal", {}) or {}
        node = terminal.get("node", {}) or {}
        reasons.append(f"Zielknoten erreicht: {node.get('label') or terminal.get('node_type') or 'terminal'}")
        next_action = "Zielknoten/Anleitung ausgeben"
        status = "sicher erkannt" if score >= 0.65 else "teilweise erkannt"
    else:
        risks.append(result.get("message") or "Graph konnte keine sichere nächste Aktion bestimmen.")
        next_action = "Support-Fallback oder Graph prüfen"
        status = "kein Treffer"

    if path:
        reasons.append(f"Genommener Pfad enthält {len(path)} Knoten/Schritte.")
    if edges:
        reasons.append(f"{len(edges)} Kante(n) geprüft.")
    if fact_status:
        reasons.append(f"Faktenerkennung: {fact_status}")
    if reply_status:
        reasons.append(f"Antwortstatus: {reply_status}")
...
def _user_explicitly_wants_setup(user_text: str, facts: dict[str, Any]) -> bool:
    t = _normalize_response_text(user_text)
    setup_words = [
        "einrichten", "installieren", "installation", "neu installieren", "erneut installieren",
        "neu einrichten", "erneut einrichten", "komplett neu", "profil neu", "neu verbinden", "setup",
    ]
    return str(facts.get("intent", "")).lower() == "setup" or any(word in t for word in setup_words)


def _user_continues_after_profile_cleanup(user_text: str, facts: dict[str, Any], previous_facts: dict[str, Any] | None = None) -> bool:
    """Erkennt den typischen Folgefall nach Passwortänderung/Profil-Löschen.

    Beispiel: Bot empfiehlt, alte eduroam-Profile zu löschen und eduroam neu
    einzurichten. Nutzer antwortet: "Habe ich gemacht, wie soll ich fortfahren?".
    Dann darf kein Betriebssystem geraten werden; der Dialog fragt zuerst nach
    dem Gerät/Betriebssystem.
    """
    t = _normalize_response_text(user_text)
    context = dict(previous_facts or {})
    context.update(facts or {})
    cleanup_context = (
        context.get("password_recently_changed") is True
        or context.get("eduroam_profile_removed") is True
        or context.get("eduroam_profile_deleted") is True
        or context.get("eduroam_profile_status") in {"removed", "deleted"}
        or context.get("eduroam_problem_type") in {"password_changed", "stored_credentials_outdated", "existing_connection_issue"}
    )
    continuation_words = [
        "habe ich gemacht", "hab ich gemacht", "erledigt", "fertig", "gemacht",
        "wie soll ich fortfahren", "wie geht es weiter", "was jetzt", "naechster schritt",
        "nächster schritt", "weiter", "fortfahren", "neu einrichten"
    ]
    return bool(cleanup_context and any(word in t for word in continuation_words))


def _text_mentions_operating_system(user_text: str) -> bool:
    return fallback_fact_extraction.text_mentions_operating_system(user_text)


def _prepare_safe_dialog_override(user_text: str, facts: dict[str, Any], previous_facts: dict[str, Any] | None = None) -> tuple[dict[str, Any] | None, dict[str, Any], str | None]:
    """Verhindert unsichere Ausgaben und erzwingt Rückfragen bei fehlenden Kerninfos.

    Wichtigster Fall: Der Nutzer bittet nach einem Troubleshooting-Hinweis um
    Neuinstallation/Neueinrichtung. Dann darf der Bot nicht Windows/macOS/etc.
    raten, sondern muss erst das Betriebssystem abfragen.
    """
    guarded_facts = dict(facts or {})
    service_key = str(guarded_facts.get("service") or guarded_facts.get("topic") or "unknown").lower()
    system_key = _wizard_system_from_facts(guarded_facts)
    previous_system_key = _wizard_system_from_facts(previous_facts or {})
    user_text_mentions_system = _text_mentions_operating_system(user_text)
    continuation_after_cleanup = _user_continues_after_profile_cleanup(user_text, guarded_facts, previous_facts)
    explicit_setup = (
        _user_explicitly_wants_setup(user_text, guarded_facts)
        or bool(guarded_facts.get("awaiting_setup_os"))
        or continuation_after_cleanup
    )
    if continuation_after_cleanup:
        guarded_facts["intent"] = "setup"
        guarded_facts["setup_requested_after_profile_cleanup"] = True
    # Wenn vorher kein Betriebssystem bekannt war und die aktuelle Eingabe keins nennt,
    # darf auch ein LLM nicht stillschweigend Windows/macOS/etc. raten.
    if service_key in {"eduroam", "vpn"} and explicit_setup and previous_system_key == "unknown" and not user_text_mentions_system:
        system_key = "unknown"

    # 1) Ungültige oder fachlich nicht auswertbare Eingaben nicht beantworten.
    if guarded_facts.get("invalid_request"):
        return _result_for_support_fallback(guarded_facts, rule_id="dialog.invalid_request_support_fallback"), guarded_facts, "Eingabe nicht auswertbar; Support-Fallback statt Raten"

    # 2) Mehrere Dienste in einer Anfrage zuerst entflechten.
    if service_key == "mixed" or guarded_facts.get("multi_topic_query") is True:
        return _result_for_mixed_topic_clarification(guarded_facts), guarded_facts, "Mehrere Dienste erkannt; Dienst wird zuerst geklärt"

    # 3) Sauberer Unknown-Flow: zuerst nachfragen, beim wiederholten Nicht-Treffer Support-Fallback.
    if service_key in {"unknown", "", "none"}:
        previous_unknown = bool((previous_facts or {}).get("unknown_request_flow")) or int((previous_facts or {}).get("unknown_request_attempts") or 0) > 0
        return _result_for_unknown_request_flow(guarded_facts, repeated=previous_unknown), guarded_facts, (
            "Unklare Anfrage wiederholt; Support-Fallback" if previous_unknown else "Unknown-Request-Flow gestartet"
        )

    if service_key in {"eduroam", "vpn"} and explicit_setup:
        guarded_facts["pending_setup_service"] = service_key
        if system_key == "unknown":
            guarded_facts["awaiting_setup_os"] = True
            # Falls durch einen früheren Fehlmatch bereits ein falscher
            # Durchlauf offen ist, darf dieser die OS-Rückfrage nicht mehr
            # überlagern oder später die richtige Auswahl blockieren.
            stop_eduroam_walkthrough()
            service_label = _service_display_name(service_key)
            question = (
                f"Ich kann {service_label} gern neu einrichten, möchte aber keine falsche Anleitung auswählen. "
                "Welches Betriebssystem nutzt du auf dem Gerät? Bitte antworte z. B. mit Windows, macOS, Android, iOS/iPadOS, Linux oder ChromeOS."
            )
            return _result_for_ask("os", question, guarded_facts, rule_id="dialog.ask_os_before_setup"), guarded_facts, "Betriebssystem vor Schrittpaket-Auswahl abgefragt"

        package_id = _setup_package_for_service_system(service_key, system_key)
        if package_id:
            guarded_facts["awaiting_setup_os"] = False
            guarded_facts["operating_system"] = system_key
            guarded_facts["os"] = system_key
            return _result_for_show_steps(
                package_id,
                guarded_facts,
                reason=f"Sichere {service_key}-Einrichtung für {system_key}, nachdem das Betriebssystem bekannt ist.",
            ), guarded_facts, "Passendes Schrittpaket nach Betriebssystem ausgewählt"

        return _result_for_support_fallback(guarded_facts, rule_id="dialog.no_setup_package_for_system"), guarded_facts, "Kein Schrittpaket für Dienst/System gefunden"

    # 4) Dienst erkannt, aber ohne Absicht und ohne konkrete Fachaktion lieber klären.
    if _service_is_known(guarded_facts) and not _intent_is_known(guarded_facts) and not guarded_facts.get("explanation_request"):
        # Bei sehr kurzen Angaben wie "eduroam" ist eine Rückfrage sicherer als eine zufällige Anleitung.
        if len(_normalize_response_text(user_text).split()) <= 4:
            return _result_for_intent_clarification(guarded_facts), guarded_facts, "Dienst erkannt, Absicht fehlt; Rückfrage statt Raten"

    return None, guarded_facts, None
...
def _recognize_facts_for_session(
    ...
    *,
    ...
    try:
        ...
        llm_facts, guard_notes = llm_safety.filter_llm_facts(
            llm_facts,
            user_text=user_text,
            current_facts=current_facts,
            pending_fact=pending_fact,
        ...
        guard_note = f" · LLM-Guard: {len(guard_notes)} Korrektur(en)" if guard_notes else ""
        return merged, (
            f"{provider_name}-Faktenerkennung genutzt ({prompt_label}) + lokaler Guard-Fallback"
            f"{question_note}{guard_note} · Modus {mode} · Dauer {_elapsed_seconds(start)}"
        )
...
def _formulate_inference_reply(
    ...
    *,
    ...
    raw_summary = sanitize_user_response(inference_engine.renderable_summary(result))
    if not use_llm:
    ...
    start = time.perf_counter()
    try:
        text = sanitize_user_response(response_generation.formulate_rule_result(raw_summary, facts, model, provider=llm_provider, title="IT-Assistent"))
...
def _formulate_graph_reply(
    ...
    *,
    ...
    raw_summary = sanitize_user_response(decision_graph_engine.render_summary(result))
    if not use_llm:
    ...
    start = time.perf_counter()
    try:
        text = sanitize_user_response(response_generation.formulate_rule_result(raw_summary, facts, model, provider=llm_provider, title="Entscheidungsnetz-Test"))
...
def _contextual_facts_from_answer(answer_text: str, pending_fact: str | None, current_facts: dict[str, Any] | None = None, pending_question: str | None = None) -> dict[str, Any]:
    ...
    return fallback_fact_extraction.contextual_facts_from_answer(
        ...
        pending_fact=pending_fact,
        pending_question=pending_question,
        current_facts=current_facts if current_facts is not None else st.session_state.get("group_inference_session", {}).get("facts", {}),
    )
...
def _maybe_start_eduroam_walkthrough_from_facts(user_text: str, facts: dict[str, Any], *, force: bool = False) -> str | None:
    ...
    # Wenn gerade bewusst nach dem Betriebssystem gefragt wird, darf dieser
    # Fallback keinen Durchlauf aus Keyword-Treffern starten. Sonst kann eine
    # Folgeantwort wie „Habe ich gemacht ...“ über die alte Rule-Engine-
    # Erkennung wieder eine falsche macOS-Anleitung öffnen.
    if facts.get("awaiting_setup_os") is True and not force:
        return None
...
def _maybe_start_walkthrough_from_inference_result(result: dict[str, Any], facts: dict[str, Any]) -> str | None:
    ...
    Sicherheitsfix: Ein bereits aktiver Durchlauf darf ein neues, sicher
    ausgewähltes Schrittpaket nicht blockieren. Genau das konnte nach einer
    vorher falsch gestarteten macOS-Anleitung passieren: Der Nutzer nennt später
    explizit „Windows“, aber der alte macOS-Durchlauf blieb aktiv. Wenn die
    neue show_steps-Aktion ein anderes Paket auswählt, wird der alte Durchlauf
    deshalb beendet und durch das neue Paket ersetzt.
    """
    ...
    existing_walkthrough = st.session_state.get("eduroam_walkthrough") or {}
    if existing_walkthrough.get("active") and not existing_walkthrough.get("done"):
        existing_package = str(existing_walkthrough.get("package_id") or "")
        if existing_package == package_id:
            return None
        stop_eduroam_walkthrough()
...
def start_group_inference_session(
    ...
    override_result, facts, override_status = _prepare_safe_dialog_override(user_text, facts)
    result = override_result or inference_engine.run_inference(facts)
    if not _has_user_visible_action(result):
        result = _result_for_support_fallback(facts)
        override_status = override_status or "Keine sichere Regelaktion gefunden"
    ...
    if override_status:
        reply_status = " · ".join(x for x in [reply_status, override_status] if x)
    diagnosis = build_decision_diagnosis(
        user_text,
        facts,
        result,
        fact_status=status,
    ...
    st.session_state.group_inference_session = {
        ...
        "history": [
            ...
            {"role": "engine", "text": reply_text, "raw_text": inference_engine.renderable_summary(result), "diagnosis": _diagnosis_line(diagnosis)},
        ],
    }
    _attach_dialog_context(st.session_state.group_inference_session, facts, result)
...
def update_group_inference_session(
    ...
    # Wenn bereits ein Schrittpaket-Durchlauf aktiv ist, gehört eine normale
    # Ja/Nein-Antwort zu diesem Durchlauf. Ausnahme: Der Dialog hat gerade
    # explizit nach dem Betriebssystem gefragt oder wartet auf ein OS für die
    # Neueinrichtung. Dann muss eine Antwort wie „Ich habe Windows“ die
    # Inferenz fortsetzen und ggf. einen alten macOS-Durchlauf ersetzen.
    existing_walkthrough = st.session_state.get("eduroam_walkthrough") or {}
    awaiting_setup_os = bool(session.get("facts", {}).get("awaiting_setup_os")) or str(pending_fact or "").lower() in {"os", "operating_system"}
    local_os_facts_for_route = {}
    if existing_walkthrough.get("active") and not existing_walkthrough.get("done") and awaiting_setup_os:
        local_os_facts_for_route = fallback_fact_extraction.recognize_with_context(
            answer_text,
            current_facts=session.get("facts", {}),
            pending_fact=pending_fact,
            pending_question=pending_question,
        )
        answered_system = _wizard_system_from_facts(local_os_facts_for_route)
        if answered_system != "unknown":
            stop_eduroam_walkthrough()
        elif _handle_active_walkthrough_answer(answer_text, session):
            return
    elif _handle_active_walkthrough_answer(answer_text, session):
        return
    ...
    contextual_facts = _contextual_facts_from_answer(answer_text, pending_fact, session.get("facts", {}), pending_question)
    combined_new_facts = _merge_facts_contextual(new_facts, contextual_facts)

    previous_facts = dict(session.get("facts", {}) or {})
    # Wenn gerade das Betriebssystem abgefragt wurde, muss eine explizite
    # Antwort wie „Windows“ alte, zuvor fälschlich gespeicherte OS-Fakten
    # überschreiben dürfen. Der normale Kontext-Merge hält OS-Werte bewusst
    # sticky, damit sie nicht durch Nebensätze verloren gehen. Für eine
    # direkte OS-Rückfrage ist dieses Sticky-Verhalten aber falsch.
    answered_system = _wizard_system_from_facts(combined_new_facts)
    if (str(pending_fact or "").lower() in {"os", "operating_system"} or previous_facts.get("awaiting_setup_os")) and answered_system != "unknown":
        previous_facts.pop("os", None)
        previous_facts.pop("operating_system", None)
        previous_facts.pop("system", None)
        combined_new_facts["os"] = answered_system
        combined_new_facts["operating_system"] = answered_system
        combined_new_facts["awaiting_setup_os"] = False

    facts = _merge_facts_contextual(previous_facts, combined_new_facts)
    override_result, facts, override_status = _prepare_safe_dialog_override(answer_text, facts, previous_facts=session.get("facts", {}))
    result = override_result or inference_engine.run_inference(facts)
    if not _has_user_visible_action(result):
        result = _result_for_support_fallback(facts)
        override_status = override_status or "Keine sichere Regelaktion gefunden"
    ...
    session["decision_diagnosis"] = diagnosis
    ...
    _attach_dialog_context(session, facts, result)
    session.setdefault("history", []).append({"role": "user", "text": answer_text, "new_facts": combined_new_facts, "answered_fact": pending_fact})
    session["history"].append({"role": "engine", "text": reply_text, "raw_text": inference_engine.renderable_summary(result), "diagnosis": _diagnosis_line(diagnosis)})
    st.session_state.group_inference_session = session
    walkthrough_status = _maybe_start_walkthrough_from_inference_result(result, facts)
    if not walkthrough_status:
...
def start_graph_test_session(
    ...
    diagnosis = build_graph_diagnosis(user_text, facts, result, graph, fact_status=status, reply_status=reply_status)
    st.session_state.graph_test_session = {
        ...
        "history": [
            {"role": "user", "text": user_text},
            {"role": "graph", "text": reply_text, "raw_text": decision_graph_engine.render_summary(result), "diagnosis": _diagnosis_line(diagnosis)},
        ],
    }
    _attach_dialog_context(st.session_state.graph_test_session, facts, result)
...
def update_graph_test_session(
    ...
    diagnosis = build_graph_diagnosis(answer_text, facts, result, graph, fact_status=status, reply_status=reply_status)
    session["facts"] = facts
    session["result"] = result
    session["reply_text"] = reply_text
    session["reply_status"] = reply_status
    ...
    _attach_dialog_context(session, facts, result)
    session.setdefault("history", []).append({"role": "user", "text": answer_text, "new_facts": combined_new_facts, "answered_fact": pending_fact})
    session["history"].append({"role": "graph", "text": reply_text, "raw_text": decision_graph_engine.render_summary(result), "diagnosis": _diagnosis_line(diagnosis)})
    st.session_state.graph_test_session = session
    _maybe_start_eduroam_walkthrough_from_graph_result(result)
