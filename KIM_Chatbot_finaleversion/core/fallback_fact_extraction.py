...
def _contains_term(text: str, term: str) -> bool:
    """Sichere Term-Erkennung mit Wortgrenzen.

    Wichtig für kurze Aliase wie "mac", "pc" oder "ios": Diese dürfen
    nicht als Teil eines normalen Wortes erkannt werden (z. B. "gemacht"
    enthält die Buchstabenfolge "mac", meint aber kein macOS-Gerät).
    """
...
def detect_operating_system(text: str) -> str | None:
    """Erkennt Betriebssysteme nur über eindeutige Tokens/Aliase."""
    t = normalize_text(text or "")
    os_aliases = [
        ("windows", ["windows", "win10", "win11", "pc"]),
        ("macos", ["macos", "mac os", "macbook", "apple", "osx", "os x", "mac"]),
        ("linux", ["linux"]),
        ("android", ["android"]),
        ("ipados", ["ipados", "ipad"]),
        ("ios", ["ios", "iphone"]),
    ...
    for system_key, aliases in os_aliases:
        if _has_any_term(t, aliases):
            return system_key
    return None


def text_mentions_operating_system(text: str) -> bool:
    return detect_operating_system(text) is not None


def _service_mentions(t: str) -> set[str]:
    services: set[str] = set()
    if "eduroam" in t or ("wlan" in t and "vpn" not in t):
        services.add("eduroam")
    if "vpn" in t or "cisco" in t or "secure client" in t:
        services.add("vpn")
    if any(x in t for x in ["mfa", "2fa", "2 fas", "2fas", "authenticator", "zweiter faktor", "zwei-faktor", "totp", "einmalcode", "otp"]):
        services.add("mfa")
    if any(x in t for x in ["benutzerkonto", "account", "passwort", "benutzername", "idm", "kennwort", "login"]):
        services.add("user_account")
    return services


def _looks_like_troll_or_nonsense(t: str) -> bool:
    if not t or len(t.strip()) < 3:
        return False
    known = [
        "eduroam", "vpn", "mfa", "2fa", "authenticator", "code", "token", "wlan", "wifi",
        "internet", "konto", "account", "passwort", "benutzer", "login", "ilias", "etherpad",
        "bibliothek", "datenbank", "kim", "zertifikat", "profil", "client", "windows", "mac", "android", "ios",
    ]
    if _has_any(t, known):
        return False
    # Sehr kurze Begriffe wie "hilfe" sollen nicht als Troll abgewiesen werden.
    if t in {"hilfe", "help", "problem", "geht nicht", "klappt nicht"}:
        return False
    nonsense_markers = ["asdf", "qwertz", "qwerty", "blabla", "bla bla", "hahaha", "lol lol", "test test", "xyzxyz", "123456789"]
    if _has_any(t, nonsense_markers):
        return True
    alnum = sum(ch.isalnum() or ch.isspace() for ch in t)
    if len(t) >= 12 and alnum / max(len(t), 1) < 0.55:
        return True
    words = [w for w in t.split() if w]
    if len(words) >= 4 and len(set(words)) <= 2 and not _has_any(t, ["geht", "nicht", "hilfe"]):
        return True
    return False
...
BOOLEAN_FACTS = {
    ...
    "other_wifi_visible", "multi_topic_query", "invalid_request", "profile_recreation_required",
...
def _base_facts_from_text(text: str) -> dict[str, Any]:
    ...
    if _looks_like_troll_or_nonsense(t):
        facts["invalid_request"] = True
        facts["intent"] = "invalid_or_unclear"
        return facts

    mentions = _service_mentions(t)
    service_mentions = {m for m in mentions if m != "user_account"}
    vpn_mfa_as_vpn_problem = (service_mentions == {"vpn", "mfa"}) and _has_any(t, ["für vpn", "fuer vpn", "beim vpn", "im vpn", "vpn-code", "vpn code", "vpn-login", "vpn login"])
    if len(service_mentions) > 1 and not vpn_mfa_as_vpn_problem:
        facts["multi_topic_query"] = True
        facts["mentioned_services"] = sorted(service_mentions)
        facts["topic"] = "mixed"
        facts["service"] = "mixed"
        if "oder" in t:
            facts["multi_topic_relation"] = "or"
        elif "und" in t or "," in t:
            facts["multi_topic_relation"] = "and"
    elif vpn_mfa_as_vpn_problem:
        facts["topic"] = "vpn"
        facts["service"] = "vpn"
        facts["intent"] = "troubleshooting"
        facts["mfa_configured"] = True
    ...
    if _has_any(t, ["unterschied zwischen vpn und eduroam", "vpn und eduroam unterschied", "unterschied vpn eduroam"]):
        facts["explanation_request"] = "vpn_eduroam_difference"
        facts["multi_topic_query"] = True
        facts["topic"] = "mixed"
        facts["service"] = "mixed"

    if facts.get("topic") != "mixed":
        if "eduroam" in t or ("wlan" in t and "vpn" not in t):
            facts["topic"] = "eduroam"
            ...
            facts["topic"] = "vpn"
            facts["service"] = "vpn"
        elif _has_any(t, ["mfa", "2fa", "2 fas", "2fas", "authenticator", "zweiter faktor", "zwei-faktor", "totp", "otp", "einmalcode"]):
            facts["topic"] = "mfa"
            facts["service"] = "mfa"
        elif _has_any(t, ["benutzerkonto", "account", "passwort", "benutzername", "idm", "kennwort", "login"]):
            facts["topic"] = "user_account"
            facts["service"] = "user_account"

    if _has_any(t, ["ilias", "etherpad", "etherpad-lite", "etherpad lite"]):
        facts["user_request"] = "ilias_etherpad"
        if not facts.get("topic") or facts.get("topic") == "unknown":
            facts["topic"] = "other_service"
            facts["service"] = "other_service"
    ...
    if _has_any(t, ["einrichten", "installieren", "installation", "setup", "verbinden", "aktivieren", "erstellen"]):
        if not facts.get("service_previously_used"):
            facts["intent"] = "setup"
        if facts.get("multi_topic_query"):
            facts["multi_service_setup"] = True
    ...
    detected_os = detect_operating_system(t)
    if detected_os:
        facts["os"] = detected_os
        facts["operating_system"] = detected_os

    if _has_any(t, ["handy", "smartphone", "telefon", "mobile", "mobil"]):
        facts["device_type"] = "smartphone"
        if facts.get("topic") == "mfa" or _has_any(t, ["mfa", "2fa", "2fas", "authenticator", "code"]):
            facts["mfa_token_device"] = "smartphone"
    if _has_any(t, ["laptop", "notebook"]):
        facts["device_type"] = "laptop"
    elif _has_any(t, ["pc", "computer"]):
        facts["device_type"] = "pc"
    ...
    if _mentions_operating_system_help(t) or _has_any(t, ["was ist ein betriebssystem", "was ist betriebssystem", "was ist ein betriebsystem", "was ist betriebsystem", "was bedeutet betriebssystem", "betriebssystem?", "was ist ein os"]):
        ...
        if not text_mentions_operating_system(t):
            facts.pop("os", None)
    ...
    if facts.get("topic") == "mfa" and facts.get("intent") == "setup":
        if _has_any(t, ["handy", "smartphone", "2fas", "authenticator app", "app"]):
            facts["mfa_token_device"] = facts.get("mfa_token_device") or "smartphone"
            facts["mfa_setup_frontend_device"] = facts.get("mfa_setup_frontend_device") or "pc_or_browser"
    ...
    if _has_any(t, ["andere wlans sichtbar", "andere wlan sichtbar", "andere netze sichtbar", "andere netzwerke sichtbar", "andere wlans sind sichtbar", "andere wlans sind alle sichtbar"]):
        facts["other_wifi_visible"] = True
        facts["wifi_available"] = True
        facts["wifi_enabled"] = True
        facts["wlan_enabled"] = True
        # Im Kontext einer eduroam-Sichtbarkeitsfrage bedeutet das: WLAN funktioniert grundsätzlich, nur eduroam fehlt.
        if facts.get("topic") in {"eduroam", None} or "eduroam" not in t:
            facts["eduroam_visible"] = False
            facts["problem_type"] = "eduroam_not_visible"
            facts["intent"] = "troubleshooting"

    if _has_any(t, ["findet eduroam nicht", "eduroam nicht finden", "eduroam nicht gefunden", "eduroam taucht nicht auf", "eduroam wird nicht angezeigt", "eduroam fehlt", "eduroam nicht sichtbar", "nicht in der wlan liste", "nicht in der liste"]):
        facts["topic"] = "eduroam"
        facts["service"] = "eduroam"
        facts["intent"] = "troubleshooting"
        facts["eduroam_visible"] = False
        ...
        facts["source_type"] = "troubleshooting"
        facts["skip_initial_prerequisites"] = True
        # Sichtbarkeit von WLAN-Netzen ist unabhängig von Account/Passwort.
        facts["account_activated"] = True
        facts["account_exists"] = True
        facts["username_known"] = True
        facts["kuerzel_known"] = True
    ...
    if _has_any(t, ["eduroam eingerichtet", "eduroam installiert", "profil installiert", "profil eingerichtet"]):
        facts["eduroam_profile_installed"] = True
        facts["eduroam_profile_configured"] = True
        facts["service_previously_used"] = True
        facts["service_previously_worked"] = True
    ...
    if _has_any(t, ["eduroam profil gelöscht", "eduroam-profil gelöscht", "profil gelöscht", "profil geloescht", "profil entfernt", "eduroam gelöscht", "eduroam geloescht"]):
        facts["topic"] = "eduroam"
        facts["service"] = "eduroam"
        facts["intent"] = "setup"
        facts["eduroam_profile_installed"] = False
        facts["eduroam_profile_configured"] = False
        facts["profile_recreation_required"] = True
        facts["service_previously_used"] = True
        facts["skip_initial_prerequisites"] = True

    if _has_any(t, ["zertifikatswarnung", "zertifikatwarnung", "zertifikatsfehler", "zertifikat fehler", "zertifikat ist falsch", "zertifikat abgelaufen", "sicherheitswarnung"]):
    ...
    if _has_any(t, ["kein internet", "ohne internet", "internet geht nicht", "internet funktioniert nicht", "komme nicht ins netz", "nicht ins netz", "offline"]):
        ...
        facts["intent"] = "troubleshooting"
        if facts.get("topic") == "eduroam" or "eduroam" in t:
            facts["topic"] = "eduroam"
            facts["service"] = "eduroam"
            facts["eduroam_connected"] = True
            facts["connection_successful"] = True
            ...
            facts["kuerzel_known"] = True
            facts["password_known"] = True
        if facts.get("topic") == "vpn" or "vpn" in t:
            facts["topic"] = "vpn"
            facts["service"] = "vpn"
            facts["vpn_connected"] = True
            facts["vpn_tunnel_status"] = "connected"
            facts["vpn_problem_type"] = "connected_no_internet"
    ...
    if _has_any(t, ["code abgelaufen", "mfa code abgelaufen"]) or ("code" in t and "abgelaufen" in t):
        facts["mfa_code_status"] = "expired"
    if _has_any(t, ["code ungueltig", "code ungültig", "falscher code", "mfa code falsch"]) or ("code" in t and _has_any(t, ["falsch", "ungueltig", "ungültig", "abgelehnt"])):
    ...
    # VPN-/MFA-spezifische Fehlerbilder
    if facts.get("topic") == "vpn" and _has_any(t, ["mfa", "2fa", "2 fas", "authenticator", "code", "einmalcode", "otp", "zweites kennwort", "zweiten kennwort", "zwei-faktor"]):
        facts["mfa_configured"] = True
        facts["two_fa_ready"] = True
        facts["mfa_required_for_vpn_login"] = True
        facts["problem_type"] = facts.get("problem_type") or "mfa_failed"
        if not facts.get("mfa_code_status"):
            facts["mfa_code_status"] = UNKNOWN
    if _has_any(t, ["zweites kennwort", "zweiten kennwort", "zweites passwort", "second password"]):
        facts["topic"] = "vpn"
        facts["service"] = "vpn"
        facts["mfa_required_for_vpn_login"] = True
        facts["mfa_code_status"] = UNKNOWN
        facts["problem_type"] = "mfa_required"
    if _has_any(t, ["code geht nicht", "code klappt nicht", "mfa code ist falsch", "2fa code ist falsch", "einmalcode nimmt", "otp funktioniert nicht", "2fa code", "mfa code"]):
        if facts.get("topic") not in {"vpn", "mixed"}:
            facts["topic"] = "mfa"
            facts["service"] = "mfa"
        facts["intent"] = "troubleshooting"
        facts["mfa_configured"] = True
        facts["mfa_code_status"] = facts.get("mfa_code_status") or "invalid"
        facts["mfa_problem_type"] = "code_rejected"
    if _has_any(t, ["code für vpn", "code fuer vpn", "vpn-code", "vpn code", "2fa-code für vpn", "2fa-code fuer vpn", "mfa-code für vpn", "mfa-code fuer vpn"]):
        facts["topic"] = "vpn"
        facts["service"] = "vpn"
        facts["intent"] = "troubleshooting"
        facts["mfa_configured"] = True
        facts["mfa_required_for_vpn_login"] = True
        facts["problem_type"] = "mfa_failed"
    if _has_any(t, ["handy weg", "handy verloren", "kein zugriff mehr auf zwei-faktor", "kein zugriff mehr auf 2fa", "kein zugriff mehr auf mfa", "authenticator weg", "token weg", "token verloren"]):
        facts["topic"] = "mfa" if facts.get("topic") not in {"vpn"} else facts.get("topic")
        facts["service"] = facts.get("topic")
        facts["intent"] = "troubleshooting"
        facts["token_lost"] = True
        facts["mfa_code_status"] = "missing"
        facts["mfa_problem_type"] = "token_lost"
    if _has_any(t, ["qr-code nicht scannen", "qr code nicht scannen", "qr-code klappt nicht", "qr code klappt nicht", "scannen klappt nicht"]):
        facts["topic"] = "mfa"
        facts["service"] = "mfa"
        facts["intent"] = "troubleshooting"
        facts["mfa_problem_type"] = "qr_scan_failed"
    if facts.get("topic") == "vpn" and _has_any(t, ["client nicht öffnen", "client nicht oeffnen", "vpn client nicht öffnen", "vpn client startet nicht", "vpn startet nicht", "bleibt beim start hängen", "bleibt beim start haengen"]):
        facts["intent"] = "troubleshooting"
        facts["vpn_problem_type"] = "client_start_failed"
        facts["vpn_client_installed"] = True
    if facts.get("topic") == "vpn" and _has_any(t, ["installer bricht ab", "installation bricht ab", "vpn installation fehler", "vpn installiert nicht"]):
        facts["intent"] = "troubleshooting"
        facts["vpn_problem_type"] = "installer_failed"
        facts["vpn_client_installed"] = False
    if facts.get("topic") == "vpn" and _has_any(t, ["benutzername ist falsch", "benutzername falsch", "falscher benutzername", "login sagt mein benutzername"]):
        facts["intent"] = "troubleshooting"
        facts["vpn_problem_type"] = "username_rejected"
        facts["username_format_correct"] = False
    if facts.get("topic") == "vpn" and _has_any(t, ["passwort nicht akzeptiert", "passwort wird nicht akzeptiert", "passwort falsch", "vpn akzeptiert mein passwort nicht"]):
        facts["intent"] = "troubleshooting"
        facts["vpn_problem_type"] = "password_rejected"
        facts["password_known"] = True
    ...
    if facts.get("mfa_code_status") in {"expired", "invalid", "missing", "available"}:
        facts["intent"] = facts.get("intent") or "troubleshooting"
