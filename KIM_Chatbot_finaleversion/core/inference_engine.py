...
def _compact_user_actions(actions: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    Generische Klärfragen werden entfernt, wenn anschließend eine spezifische
    Fachantwort oder Rückfrage greift.
    """
    if not actions:
        return actions
    has_specific_user_action = any(a.get("type") in {"answer", "recommend", "show_steps", "redirect_topic"} for a in actions)
    if has_specific_user_action and len([a for a in actions if a.get("type") != "set_fact"]) > 1:
        filtered: list[dict[str, Any]] = []
        for a in actions:
            text = str(a.get("text", ""))
            if a.get("type") == "ask" and a.get("fact") == "intent" and "Möchtest du etwas einrichten" in text:
                continue
            filtered.append(a)
        actions = filtered
...
def renderable_summary(result: dict[str, Any]) -> str:
    ...
    if not texts:
        return "Ich konnte noch keine passende Lösung sicher auswählen. Beschreibe bitte kurz, ob es um eduroam, VPN, MFA oder dein Benutzerkonto geht und was genau nicht funktioniert."
