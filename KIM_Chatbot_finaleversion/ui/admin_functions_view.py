...
TYPES = ["Routing", "Precondition", "Pre-Condition-Hilfe", "Setup", "Auth", "Authentifizierung", "Connection", "Verbindungstest", "Troubleshooting", "Escalation", "Completion", "Antwortbaustein", "Abschluss"]
...
def admin_functions_answers() -> None:
    render_view_title(
        ...
        "Funktionen beschreiben, was der Bot tut oder ausgibt. Regeln entscheiden, wann eine Funktion genutzt wird. Hier pflegst du Anzeigenamen, Typen, Antworttexte und gesetzte Wissensbausteine; die technische ID bleibt stabil.",
    )
    ...
    with tab_functions:
        ...
        c4.metric("verknüpfte Regeln", len({x.get("used_in_rule") or x.get("used_in") for x in functions if x.get("used_in_rule") or x.get("used_in")}))
            ...
            [
                {
                    ...
                    "Liest Facts": x.get("input_facts"),
                    "Setzt Facts": x.get("sets_facts"),
                    "Genutzt in Regel": x.get("used_in_rule", x.get("used_in", "")),
                    "Zweck": x.get("purpose"),
                }
                for x in visible
            ],
        ...
        with st.form(f"function_form_{suffix}"):
            ...
            namespace = st.text_input("Namespace", value="" if selected is None else selected.get("namespace", ""), key=f"function_namespace_{suffix}")
            category = st.text_input("Kategorie", value="" if selected is None else selected.get("category", ""), key=f"function_category_{suffix}")
            st.markdown("### Wirkung der Funktion")
        ...
        if save:
            try:
                item = {
                    ...
                    "note": note.strip(),
                    "used_in_rule": selected.get("used_in_rule", selected.get("used_in", "")) if selected else "",
                    "active": active,
