...
from ui.common import esc, json_area, render_answer, render_view_title
...
def _graph_toolbar(graph: dict[str, Any], flow_state: Any = None) -> None:
    graph_id = graph.get("id")
    selected_type, selected_id = _current_selection(graph_id)
    # Die Toolbar wird direkt als Streamlit-Spalten gerendert. Ein offenes HTML-Wrapper-div
    # würde in Streamlit nur als leere Box erscheinen, weil Widgets nicht in ein
    # zuvor geöffnetes HTML-div hineingerendert werden.
...
def _graph_meta_editor(selected_graph: dict[str, Any] | None) -> None:
    with st.expander("Graph-Stammdaten", expanded=selected_graph is None):
        with st.form("admin_graph_meta_form"):
            ...
            description = st.text_area("Beschreibung", value="" if selected_graph is None else selected_graph.get("description", ""), height=110, key="admin_graph_description")
...
def admin_decision_graphs() -> None:
    render_view_title(
        "Admin · Grafischer Entscheidungsnetz-Editor",
    ...
    with right:
        # Kein offenes HTML-div als Wrapper verwenden: Streamlit rendert Widgets nicht
        # innerhalb solcher Wrapper, dadurch entstand über "Auswahl" eine leere Box.
