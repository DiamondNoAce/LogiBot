...
def admin_knowledge_model() -> None:
    ...
    with tab_refs:
        ...
        if not nodes:
            ...
            st.write("**Verwendete Wissensbausteine:**")
            if fact_keys:
            ...
            if c1.button("Wissensbaustein anzeigen", key="km_go_conditions"):
                st.session_state["pending_view"] = "Admin: Wissensbausteine"
                st.rerun()
            ...
            if c3.button("Funktion bearbeiten", key="km_go_functions"):
                st.session_state["pending_view"] = "Admin: Funktionen & Antworten"
                st.rerun()
