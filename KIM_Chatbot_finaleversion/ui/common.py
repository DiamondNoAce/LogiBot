    ...
    st.markdown(
        ...
        <style>
            ...
            .view-help-tooltip {
                ...
                color: var(--hoh-text);
                border: 1px solid #BBD7EA;
                border-left: 5px solid var(--hoh-blue-2);
                border-radius: 10px;
                padding: 0.8rem 0.9rem;
                box-shadow: 0 8px 24px rgba(0, 59, 112, 0.16);
                font-size: 13px;
                line-height: 1.5;
                text-align: left;
            }
            .view-help-tooltip-title {
                color: var(--hoh-blue);
                font-weight: 800;
                margin-bottom: 0.25rem;
            }
            .view-help-wrap:hover .view-help-tooltip,
            .view-help-wrap:focus-within .view-help-tooltip {
                visibility: visible;
                opacity: 1;
            }
            .view-help-tooltip-anchor {
                display: inline-flex;
                margin: 0 0 0.8rem 0;
            }
            ...
            /* Sichtbare Formularfelder: Streamlit/BaseWeb rendert Eingaben nicht als normales input-Feld,
               sondern mit verschachtelten Containern. Deshalb werden Container UND Text explizit gestylt. */
            ...
            div[data-testid="stSelectbox"] div[data-baseweb="select"] > div {
                ...
                box-shadow: 0 1px 2px rgba(0, 59, 112, 0.04) !important;
            ...
            div[data-testid="stSelectbox"] div[data-baseweb="select"] > div:focus-within {
                border-color: var(--hoh-blue-2) !important;
                box-shadow: 0 0 0 2px rgba(0, 105, 170, 0.14) !important;
            }
            ...
            div[data-baseweb="select"] input {
                color: var(--hoh-text) !important;
                -webkit-text-fill-color: var(--hoh-text) !important;
...
def render_hohenheim_header(active_view: str = "", nav_map: dict[str, str] | None = None) -> None:
    ...
    # Die zusätzliche weiße Seitentitel-Box wurde entfernt, weil die einzelnen
    # Ansichten ihren Titel und ihre Kurzbeschreibung bereits selbst anzeigen.
...
def _view_help_html(title: str, text: str) -> str:
    return f"""
        <span class="view-help-wrap" tabindex="0" aria-label="Hinweis zur Ansicht">
            <span class="view-help-button">?</span>
            <span class="view-help-tooltip" role="tooltip">
                <div class="view-help-tooltip-title">{esc(title)}</div>
                <div>{esc(text)}</div>
            </span>
        </span>
    """
...
def render_view_title(title: str, help_title: str, help_text: str, *, subtitle: str | None = None, hero: bool = False) -> None:
    """Rendert den Ansichtstitel mit kleinem Fragezeichen-Hinweis statt großem Erklärungskasten."""
    title_html = (
        f'<div class="hero-title">{esc(title)}</div>'
        if hero
        else f'<h2>{esc(title)}</h2>'
    )
    subtitle_html = f'<div class="hero-subtitle">{esc(subtitle)}</div>' if subtitle else ''
    st.markdown(
        f"""
        <div class="view-title-row">
            {title_html}
            {_view_help_html(help_title, help_text)}
        </div>
        {subtitle_html}
        """,
        unsafe_allow_html=True,
...
def render_view_info(title: str, text: str) -> None:
    """Kompatibilitätsfunktion: zeigt nur noch ein kleines Fragezeichen mit Tooltip, keinen Infokasten."""
    st.markdown(
        f'<span class="view-help-tooltip-anchor">{_view_help_html(title, text)}</span>',
        unsafe_allow_html=True,
    )
