...
def _extract_post(rule: dict[str, Any]) -> list[dict[str, Any]]:
    return (rule.get("technical_metadata") or {}).get("post_conditions") or []
...
def admin_inference_rules() -> None:
    st.header("Admin · Regelverwaltung")
    render_view_info(
        "Regelverwaltung",
    ...
    with tab_table:
        ...
        search = st.text_input("Suchen", key="rule_search", placeholder="Regel-ID, Condition, Funktion oder Beschreibung")
        visible = rules
        if module_filter:
            ...
            [
                {
                    ...
                    "Action / Funktion": _action_from_rule(r),
    ...
    with tab_editor:
        ...
        with st.expander("Condition- und Funktionskatalog als Hilfe"):
            c1, c2 = st.columns(2)
            with c1:
                st.markdown("**Conditions/Facts**")
    ...
    with tab_help:
        ...
        **Editor-Logik**
        ...
        `regex`, `is_unknown`, `is_known`, `is_true`, `is_false`, `greater_than`, `less_than`, `greater_or_equal`, `less_or_equal`.
        """)
