...
def render_public_problem_session() -> None:
    ...
    if session.get("reply_status"):
        st.caption(session.get("reply_status"))
...
def render_user_view(
    ...
    st.markdown(
        '<div class="hero-subtitle">Beschreibe dein IT-Problem, suche eine Anleitung oder wähle ein häufiges Thema aus. Technische Tests sind in die Ansicht „Tests & Diagnose“ verschoben.</div>',
        unsafe_allow_html=True,
    )
    render_view_info(
        "Nutzeroberfläche",
        "Diese Ansicht ist bewusst einfach gehalten: Nutzer sehen keine Regeltraces, Facts oder Entscheidungsnetz-Debugdaten. Die Rule Engine führt im Hintergrund durch Rückfragen und passende Anleitungen.",
    )
