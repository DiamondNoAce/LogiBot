...
Das Wissensmodell beschreibt den gemeinsamen Kern über MFA, VPN, eduroam und Support.
Nutze dessen Namespaces und Fact-Keys, wenn die Nutzereingabe dazu passt:
{knowledge_context}

Faktkatalog:
{json.dumps(catalog, ensure_ascii=False, indent=2)}

Dienstübergreifende globale Bausteine:
{json.dumps(global_blocks, ensure_ascii=False, indent=2)}

Pflichtfelder:
topic, service, intent, os, operating_system, account_exists, account_activated, account_status, account_locked, username_known, password_known, credentials_valid, password_recently_changed, internet_available, network_context, wifi_available, wifi_enabled, eduroam_visible, eduroam_profile_configured, connection_attempt_status, internet_access_available, problem_type, two_fa_required, two_fa_ready, mfa_problem_type, mfa_app_available, mfa_recovery_available, mfa_challenge_approved, campus_network_available, vpn_client_installed, vpn_endpoint_reachable, login_form_requested, vpn_tunnel_status, internal_resource_accessible, mfa_configured, eduroam_connected, username_format_correct, email_used_as_username, needs_human_support
"""
    response = chat([
        {"role": "system", "content": prompt.strip()},
        {"role": "user", "content": user_text.strip()},
