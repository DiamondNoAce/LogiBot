...
def render_user_view(use_ollama: bool, model: str, fallback: bool) -> None:
    ...
    if mode == "Anleitung suchen":
        ...
        if "user_inference_input_counter" not in st.session_state:
            st.session_state.user_inference_input_counter = 0
        inference_input_key = f"user_inference_text_{st.session_state.user_inference_input_counter}"
        ...
        if col_run.button(button_label, key="user_inference_submit"):
            try:
                ...
                if not cleaned_text:
                    ...
                    update_group_inference_session(cleaned_text, use_ollama, model, fallback)
                    _bump_counter("user_inference_input_counter")
                    st.rerun()
                else:
                    start_group_inference_session(cleaned_text, use_ollama, model, fallback)
                    _bump_counter("user_inference_input_counter")
                    st.rerun()
            except Exception as e:
        ...
        if not graphs:
            ...
            if "user_graph_input_counter" not in st.session_state:
                st.session_state.user_graph_input_counter = 0
            graph_input_key = f"user_graph_text_{st.session_state.user_graph_input_counter}"
            ...
            if col_run.button(button_label, key="user_graph_submit"):
                try:
                    ...
                    if not cleaned_text:
                        ...
                        update_graph_test_session(cleaned_text, use_ollama, model, fallback)
                        _bump_counter("user_graph_input_counter")
                        st.rerun()
                    else:
                        start_graph_test_session(graph, cleaned_text, use_ollama, model, fallback)
                        _bump_counter("user_graph_input_counter")
                        st.rerun()
                except Exception as e:
