...
def facts_from_text(user_text: str) -> dict[str, Any]:
    ...
    if any(x in t for x in ["benutzername bekannt", "kuerzel bekannt", "kürzel bekannt", "ich kenne mein kuerzel", "ich kenne mein kürzel"]):
        facts["username_known"] = True
    if any(x in t for x in ["benutzername unbekannt", "kenne mein kuerzel nicht", "kenne mein kürzel nicht"]):
        facts["username_known"] = False
    if any(x in t for x in ["passwort bekannt", "ich kenne mein passwort", "kenne mein passwort", "kenne mein kuerzel und passwort", "kenne mein kürzel und passwort", "kennwort bekannt"]):
        facts["password_known"] = True
    if any(x in t for x in ["passwort vergessen", "kennwort vergessen", "passwort unbekannt", "kenne mein passwort nicht"]):
